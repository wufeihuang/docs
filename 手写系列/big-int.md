...
> jsbi 库：https://github.com/GoogleChromeLabs/jsbi
> 
> 在 JS 中，对于大于1的数，整数位超过21位（10进制）会被自动转成科学计数法；对于小于1的数，紧跟着小数点后超过6个0时也会被转成科学计数法表示。
>
> 最大安全数字是 Number.MAX_SAFE_INTEGER，即 2**53，16位10进制长度。

自定义 BigInt 实现，支持大数的加、减、乘、除、取余和比较运算。

用法：

```js
// 创建：
const a = new BigInt2('2000000000001111')
const b = new BigInt2('100000')

// 运算
BigInt2.add(a, b) // 加法
BigInt2.subtract(a, b) // 减法
BigInt2.multiply(a, b) // 乘法
BigInt2.divide(a, b) // 除法
BigInt2.mod(a, b) // 取余
BigInt2.compare(a, b) // 比较，返回 -1、0、1
```
